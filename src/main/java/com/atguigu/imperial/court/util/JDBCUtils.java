...
/**
 * 功能1：从数据源获取数据库连接
 * 功能2：从数据库获取到数据库连接，绑定到本地线程（借助threadLocal）
 * 功能3：释放线程时和本地线程解除绑定
 */
public class JDBCUtils {
    ...
    //由于ThreadLocal 对象需要作为绑定数据时的 k-v 对中的key，所以要保证唯一性。
    //加static声明静态资源即可保证唯一性
    ...
    static {
        try {
            //操作思路分析：
            //从jdbc.properties文件中读取数据库信息
            //为了确保程序代码的可移植性，需要基于一个确定的基准来读取这个文件
            //确定的基准：类路径的根目录。resources目录下的内容经过构建操作中的打包操作后会确定放在 WEB-INF/classes 目录
            //WEB-INF/classes 目录存放编译好的 *.class 字节码文件，所以这个目录我们就称之为类路径。
            //类路径无论是在本地运行还是在服务器端运行都是一个确定的基准。
            //操作具体代码：
            //1.获取当前类的类加载器
